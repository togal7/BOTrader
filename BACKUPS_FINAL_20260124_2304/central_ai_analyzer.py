...
class CentralAIAnalyzer:
    ...
    async def analyze_for_ai_signals(self, symbol: str, main_tf: str, exchange: str = 'mexc') -> dict:
        """
        ...
        # Analiza głównego TF
        main_analysis = await self.analyze_pair_full(symbol, exchange, main_tf, 'ai_signals')
        ...
        if htf:
            try:
                htf_analysis = await self.analyze_pair_full(symbol, exchange, htf, 'ai_signals_htf')
        ...
        if ltf:
            try:
                ltf_analysis = await self.analyze_pair_full(symbol, exchange, ltf, 'ai_signals_ltf')
        ...
        # Enhanced confidence
        final_confidence = min(base_confidence + mtf_boost, 95)
