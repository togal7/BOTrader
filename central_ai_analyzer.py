...
IMPORTANT: Respond in {ai_language} language for all reasoning text. Use professional trading terminology in Polish."""
    ...
    async def _generate_trading_signal(self, technical, structure, volume, sentiment, ai_reasoning, news, ewt_analysis=None):
        """
        ZAAWANSOWANE GENEROWANIE SYGNAŁU
        Multi-TF Scoring System + EWT + Volume Profile + Ichimoku
        """
        direction = "NEUTRAL"
        confidence = 50
        reasoning = []
        score = 0  # Max score: 11 points
        
        # === SCORING SYSTEM ===
        
        # 1. EWT Analysis (max 2 pts)
        if ewt_analysis and ewt_analysis.get('bias') != 'NEUTRAL':
            ewt_conf = ewt_analysis.get('confidence', 0)
            if ewt_conf >= 70:
                score += 2
                reasoning.append(f"EWT: {ewt_analysis['bias']} (wave {ewt_analysis.get('primary', {}).get('current_wave', '?')})")
            elif ewt_conf >= 50:
                score += 1
                reasoning.append(f"EWT: Weak {ewt_analysis['bias']}")
        
        # 2. Volume Profile + VWAP (max 2 pts)
        vp_data = volume.get('vp_data', {})
        vp_bias = vp_data.get('bias', 'NEUTRAL')
        
        if vp_bias != 'NEUTRAL':
            score += 1
            reasoning.append(f"VP: {vp_bias}")
        
        current_price = volume.get('current_price', technical.get('price', 0))
        vwap = vp_data.get('vwap', 0)
        if vwap > 0 and current_price > vwap:
            score += 1
            reasoning.append("Price > VWAP")
        elif vwap > 0 and current_price < vwap:
            score += 1
            reasoning.append("Price < VWAP")
        
        # 3. Ichimoku (max 1 pt)
        ichimoku = technical.get('ichimoku', {})
        if ichimoku.get('signal') != 'NEUTRAL':
            score += 1
            reasoning.append(f"Ichimoku: {ichimoku['signal']}")
        ...
        # 5. EMA Trend (max 2 pts)
        ema_9 = technical.get('ema', {}).get('9', 0)
        ema_21 = technical.get('ema', {}).get('21', 0)
        ema_50 = technical.get('ema', {}).get('50', 0)
        
        if ema_9 > ema_21 > ema_50:
            score += 2
            reasoning.append("EMA: Strong uptrend")
        elif ema_9 < ema_21 < ema_50:
            score += 2
            reasoning.append("EMA: Strong downtrend")
        elif ema_9 > ema_21:
            score += 1
        ...
        # 6. Volume (max 1 pt)
        vol_ratio = volume.get('current_vs_avg', volume.get('ratio', 1.0))
        if vol_ratio > 1.5:
            score += 1
        ...
        biases = []
        if ewt_analysis:
            biases.append(ewt_analysis.get('bias'))
        biases.append(vp_bias)
        biases.append(ichimoku.get('signal', 'NEUTRAL'))
        
        long_count = biases.count('LONG')
        short_count = biases.count('SHORT')
        
        if long_count > short_count and score >= 1:
            direction = "LONG"
        elif short_count > long_count and score >= 1:
            direction = "SHORT"
        elif score >= 3:
        ...
        if ai_reasoning.get('confidence', 0) > 70:
            confidence += 5
        
        confidence = min(100, max(25, confidence))
        
        # === TP/SL ===
        current_price = technical.get('price', current_price)
        ...
        risk = abs(current_price - sl) if direction != "NEUTRAL" else 1
        reward = abs(tp2 - current_price) if direction != "NEUTRAL" else 1
