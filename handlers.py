...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("chart_"):
        await show_signal_chart(query, query.data.replace("chart_", ""))
        return
...
4. Wyślij USDT na adres powyżej
5. Prześlij admina screenshot TX
6. Aktywacja w ciągu 24h!

⚡ WAŻNE:
• Tylko USDT TRC20 (sieć TRON)
• Podaj swoje ID: {user_id}
...
async def ai_scan_speed_choice(query, user_id, user, timeframe, scan_size):
    """Wybór szybkości skanowania: szybkie (wskaźniki) vs dokładne (AI)"""
    text = f"""⚡ WYBIERZ TRYB SKANOWANIA
...
Który tryb?"""
    ...
    keyboard = [
        [InlineKeyboardButton('🚀 Szybkie (wskaźniki)', callback_data=f'ai_fast_{timeframe}_{scan_size}')],
        [InlineKeyboardButton('🎯 Dokładne (AI)', callback_data=f'ai_full_{timeframe}_{scan_size}')],
        [InlineKeyboardButton('⬅️ Powrót', callback_data='show_cached_scan')]
    ]
        ...
        for symbol in symbols[:total_to_scan]:
            ...
            if scanned % 10 == 0:
                try:
                    ...
                    pass
            try:
                # Sprawdź tryb skanowania
                scan_mode = user.get('scan_mode', 'full')
                
                if scan_mode == 'fast':
                    # SZYBKIE - tylko wskaźniki, bez DeepSeek
                ...
                if result:  # Akceptuj WSZYSTKO (nawet nisko)
                    if result.get('signal') != 'NEUTRAL':  # Filtruj NEUTRAL w DOKŁADNYM
                        results.append({
...
══════════════════════════════════"""
        ...
        # Przyciski
        keyboard = [
            [InlineKeyboardButton('📊 Wykres', callback_data=f'chart_{symbol}_{timeframe}')],
            [InlineKeyboardButton('⬅️ Powrót', callback_data='show_cached_scan')],
        ...
        await query.edit_message_text(
            f"❌ Błąd: {e}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⬅️ Powrót', callback_data='show_cached_scan')]])
        )
...
async def show_cached_scan(query, user_id, user):
    ...
    if not cached or len(cached) == 0:
        logger.info(f"No cache - pokazuję menu AI Signals")
        # Pokaż menu AI Signals zamiast automatycznego skanowania
        await query.answer("Brak zapisanych wyników. Wybierz skanowanie.", show_alert=True)
        await ai_signals_menu(query, user_id, user)
        return
    print(f"🔥 Showing {len(cached)} cached results")
...
async def show_pair_analysis(query, user_id, user, symbol, exchange, timeframe, context='general'):
    print(f"🔥 show_pair_analysis called: {symbol} {timeframe}")
    """
...
async def admin_signals_stats(query, user_id, user):
    ...
    # Oblicz statystyki bezpośrednio z ai_signals_history.json
    try:
...
async def show_signal_chart(query, signal_id):
    ...
    try:
        ...
        exchange = "MEXC"  # Default (uppercase required)
        ohlcv = await exchange_api.get_klines(exchange, symbol, timeframe, limit=200)
        if not ohlcv:
            await query.answer("❌ Brak danych", show_alert=True)
            return
        ...
        entry = ticker.get('last') if ticker else df['close'].iloc[-1]
        img = await chart_gen.generate_signal_chart(symbol=symbol, timeframe=timeframe, ohlcv_data=df, entry_price=entry, position_type='LONG', tp_levels=[entry*1.02, entry*1.04], sl_price=entry*0.98)
        await query.message.reply_photo(photo=img, caption=f"📊 {symbol} {timeframe}")
    except Exception as e:
        logger.error(f"Chart: {e}")
        await query.answer("❌ Błąd", show_alert=True)
